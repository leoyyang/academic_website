...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initScrollAnimations() {
        const animateElements = document.querySelectorAll(
            '.pub-list-item, .card, .stream-item, .software-card'
        );

        // 若浏览器不支持 IntersectionObserver，直接让内容全部可见，
        // 绝不把内容留在 opacity:0（否则 JS 一出问题整段空白）。
    ...
    function enhanceButtonColors() {
        const buttons = document.querySelectorAll('.btn-links .btn, .stream-item .btn');
    ...
    function optimizePerformance() {
        ...
        // 入场动画只需初始化一次；resize 时不再重跑，
        // 避免重复创建 observer 与重复切换类。
    }
        ...
        setTimeout(() => {
            ...
            const tabsHtml = `
                <div class="research-tabs-nav">
                    <button class="research-tab active" data-category="all">All</button>
                    <button class="research-tab" data-category="Media">Media</button>
                    <button class="research-tab" data-category="Labor">Labor</button>
                    <button class="research-tab" data-category="GenAI">GenAI</button>
                </div>
            `;
            
            // 在第一个publication前插入tab导航栏
            const firstPub = publications[0];
            const tabsContainer = document.createElement('div');
            tabsContainer.innerHTML = tabsHtml;
            firstPub.parentNode.insertBefore(tabsContainer.firstElementChild, firstPub);
    ...
    audioScript.src = '/js/audio-player.js';
